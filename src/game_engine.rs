...
/* I added some methods for providing a more Rustacean API */
...
impl GameEngine {
    pub fn new() -> Self {
        let mut engine = Self { //todo esto hace SIGSEV si falla el init porque para Rust ya esta creado entonces le llama a drop que llama a release
    ...
    pub fn read_event(&self) -> Input_t {
        unsafe {
            gameEngine_read_event(self)
        }
    }

    pub fn render(&self, game_state: &GameState_t) {
        unsafe {
            gameEngine_render(self, game_state);
        }
    }
}
...
impl Drop for GameEngine {
    fn drop(&mut self) {
        unsafe {
            gameEngine_release(self); //todo esto hace SIGSEV si me fallo el init porque potencialmente tengo punteros en NULL que les llamo a release
        }
    }
}
